...
This repository provides a robust, user-friendly automated testing framework for the Sefaria mobile app, powered by WebdriverIO and Appium. It enables you to run end-to-end tests on Android devices, either locally or in the cloud with BrowserStack, ensuring the app’s quality and reliability across updates. Whether you’re a first-time contributor or an experienced developer, this guide will help you quickly set up, run, and troubleshoot tests with clear, step-by-step instructions.
...
- Edit `.env` and set (open with Notepad, TextEdit, VS Code, or any text editor):
  ...
  Set `LOCAL_APP_PATH` to the **full absolute path** to your `sefaria_app.apk` file. For example:

  - **Windows:** Right-click the file, select "Properties", and copy the "Location" and filename.
...
- **If you need to clone this repository:**
  - Install [Git](https://git-scm.com/downloads) if you don't have it.
  - Clone with:

    ```sh
    git clone https://github.com/HershelT/Sefaria-Automatic-App-Tests.git
    ```
...
- **Navigate to the android directory (if not already there):**
...
You can run the automated tests on BrowserStack using GitHub Actions. This is useful for verifying your changes in a clean environment and for sharing results with your team.
...
2. **Open the Actions Tab:**
   - Go to your repository on GitHub (e.g., `https://github.com/HershelT/Sefaria-Automatic-App-Tests`).
...
4. **Trigger the Workflow Manually:**
   ...
   - If there are any input fields (e.g., for custom environment variables), fill them in as needed (usually you can leave them as default).
...
7. **Troubleshooting Failed Runs:**
   ...
   - Double-check your `.env` file in the repository and make sure all required secrets are set in the GitHub repository settings (e.g., `BROWSERSTACK_USERNAME`, `BROWSERSTACK_ACCESS_KEY`).
...
.github/
  workflows/
    browserstack-tests-android.yml
android/
  .env
  package.json
  tsconfig.json
  diff-images/
    # Contains images of failed actions in UI
  logs-test/
    # Contains test run logs
  app/
    app-release.aab
    sefaria_app.apk
    sefaria_app.apk.idsig
  testing-framework/
    components/
      display_settings.ts
      navigation.ts
...
- **GitHub Issues:** [github.com/sefaria/Sefaria-Automatic-App-Tests](https://github.com/HershelT/Sefaria-Automatic-App-Tests)
...
## Contributing & Writing Your Own Tests

Contributions are welcome! Feel free to add your own automated tests or improve existing ones. If you have ideas for new test scenarios or want to help expand coverage, please open a pull request.
