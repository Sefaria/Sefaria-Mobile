...
describe('MenuItemsMeta', () => {
   test('logged in menu items', () => {
       expect(MenuItemsMeta.getMenuItems(true)).map(x => x['title']).toStrictEqual(['profile', 'updates', 'settings', 'language', 'help', 'about', 'logout', 'donate']);
   });

   test('logged out menu items', () => {
       expect(MenuItemsMeta.getMenuItems(false)).map(x => x['title']).toStrictEqual(['register', 'login', 'updates', 'settings', 'language', 'help', 'about', 'donate'])
   });
